...
export const registerTechinicianService = async (phone, email, first_name, last_name, usertype, dealerId, otp) => {
...
export const isTechnicianSignedIn = async (tech_id) => {
...
export const approveTechnicianService = async (dealer_id, tech_id) => {
...
export const getAllApprovedTechniciansService = async (dealer_id) => {
    const existingDealer = await prisma.dealers.findUnique({
        where: {
            dealer_id,
        },
...
export const getAllDisapprovedTechniciansService = async (dealer_id) => {
    const existingDealer = await prisma.dealers.findUnique({
        where: {
            dealer_id,
        },
...
export const disapproveTechnicianService = async (dealer_id, tech_id) => {
    const existingTechnician = await prisma.technicians.findUnique({
        where: { tech_id, dealerid: dealer_id },
    });
    if (!existingTechnician)
...
export const getAllTechniciansForDealer = async (dealer_id) => {
    return await prisma.technicians.findMany({
        where: {
            dealerid: dealer_id,
        },
