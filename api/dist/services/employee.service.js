...
const generateEmployeeToken = (emp_id) => {
    return jwt.sign({ emp_id }, config.jwtkey, { expiresIn: '7d' });
};
export const employeeRegisterService = async (fullname, email, phone, password) => {
...
export const employeeLoginService = async (email, password) => {
    ...
    if (!user)
        throw new Error('Employee with this email do not exist');
...
export const getEmployeeProfileService = async (emp_id) => {
...
export const getAllApprovedEmployeesService = async () => {
    ...
    if (!approvedEmployyes)
        throw new Error('Approved employees not found');
...
export const getAllDispprovedEmployeesService = async () => {
    ...
    if (!disapprovedEmployyes)
        throw new Error('Disapproved employees not found');
    return disapprovedEmployyes;
};
export const approvedEmployeeService = async (emp_id) => {
    const existingEmployee = await prisma.employee.findUnique({
        where: {
            emp_id,
        },
...
export const disapprovedEmployeeService = async (emp_id) => {
    const existingEmployee = await prisma.employee.findUnique({
        where: {
            emp_id,
        },
