 ...
 * @access  Public
 */
export const registerDealerController = async (req, res) => {
    try {
        console.log('Incoming Dealer Registration Request:', req.body);
        // Extract required registration details from request body
        const { phone, email, first_name, last_name, otp, gstin, usertype, pncd, loc, dst, st, stcd, adr } = req.body;
        ...
        const shipping_address = pncd || loc || dst || st || stcd || adr ? { pncd, loc, dst, st, stcd, adr } : null;
        console.log('Shipping Address:', shipping_address);
        // Call service layer to register dealer
        const dealer = await dealerServices.registerDealerService(phone, email, first_name, last_name, gstin, usertype, shipping_address, apiKey, otp);
 ...
 */
export const isDealerSignedInController = async (req, res) => {
    try {
