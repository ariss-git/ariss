...
// Type definition representing the structure of incoming RMA form data
...
export class RMAService {
     ...
     */
    async rmaRequestFormService(formData: RequestForm) {
     ...
     * @returns Updated RMA record
     */
    async rmaRequestRejectedService(rma_id: string) {
        const existingRMA = await this.prismaClient.rMA.findUnique({
            where: { rma_id },
            select: { phone: true, email: true, product_name: true, status: true },
        });
     ...
     * @returns Updated RMA record
     */
    async rmaRequestResolvedService(rma_id: string) {
        const existingRMA = await this.prismaClient.rMA.findUnique({
            where: { rma_id },
            select: { phone: true, email: true, product_name: true },
        });

        if (!existingRMA) throw new Error('RMA with this id do not exists');
     ...
     * @returns RMA record
     */
    async getSingleRMAService(rma_id: string) {
     ...
     */
    async deleteRMARequestService(rma_id: string) {
        const existingRMA = await this.prismaClient.rMA.findUnique({ where: { rma_id } });
