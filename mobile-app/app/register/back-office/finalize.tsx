...
import { registerBackOffice } from '~/api/authServices';
import { useAuthStore } from '~/store/auth';

export default function BackOfficeSubmit() {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      console.log('Sending Data:', backOfficeData);

      const { data } = await registerBackOffice(backOfficeData);

      if (data.success) {
        Toast.show({
          type: 'success',
          text1: 'Congratulations! Your back office account has created.',
