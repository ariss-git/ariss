...
export const dealerProfile = async (token: string) => {
  return axios.get('http://localhost:5000/api/dealer/profile', {
    headers: {
...
export const backOfficeProfile = async (token: string) => {
  return axios.get('http://localhost:5000/api/back-office/check', {
    headers: {
      Authorization: `Bearer ${token}`, // Send token in Authorization header
    },
    withCredentials: true, // Allow credentials
...
export const technicianProfile = async (token: string) => {
  return axios.get('http://localhost:5000/api/technician/check', {
    headers: {
      Authorization: `Bearer ${token}`, // Send token in Authorization header
    },
    withCredentials: true, // Allow credentials
